...
char prevous_state;//陷入切换模式之前的状态
...
void myKN_callback()
{
	...
	if (state == 8)
	{
		if (K3 == enumKeyPress && FM_key == 0)//未选中状态进入选中状态
		{
			FM_key = 1;//获取锁
			FM_volume = 1;
			SetBeep(1500,20);
		}
		else if (K3 == enumKeyPress && FM_key == 1 && FM_volume == 1)//选中状态恢复未选中状态
		{
			FM_key = 0;//释放锁
			FM_volume = 0;
			SetBeep(1500,20);
...
{ 
	...
	else if (key_flag == 1 && K1 == enumKeyPress)
	{
		...
		{
			SetBeep(600,10);
			state=prevous_state+1;//切换到下一个模式
			transfer=1;
		}
		else 
		{
	...
	if (state == 8)
	{
		if (K2 == enumKeyPress && FM_key == 0)//未选中状态进入选中状态
		{
			FM_key = 1;//获取锁
			FM_frequency = 1;
			SetBeep(1500,20);
		}
		else if (K2 == enumKeyPress && FM_key == 1 && FM_frequency == 1)//选中状态恢复未选中状态
		{
			FM_key = 0;//释放锁
			FM_frequency = 0;
			SetBeep(1500,20);
